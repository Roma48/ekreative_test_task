(function ($) {
function node_embed_get_node(nid){
    jQuery.ajax({
        type   : "POST",
        url    : Drupal.settings.basePath + 'node-embed-edsuite-node',
        dataType: "json",
        data   : 'node_embed_nid=' + nid,
    ...
    $('#buttonEmbedFromDialog').click(function(e) {
      ...
      if( node_id != null && node_id != "" ) {
          jQuery.ajax({
              ...
              url    : Drupal.settings.basePath + 'node-embed-edsuite-node',
              dataType: "json",
              data   : 'node_embed_nid=' + node_id,
              success: function(json)
              {
                  dialog.editor_content = '' + json.node_text + '';
                  if(window.currentActiveNid && window.currentActiveNid != ""){
                      var edit_content = ""+json.node_text+"";
                      if (data.format == 'html') {
